...
class circuit {
	constructor() {
		...
		this.gate_type = "";
		this.selectedPoint = null; // Nodo seleccionado
	}

	addWire(pointA,interpoint, pointB) {
		// Crea la nueva linea
		let line = new Line(pointA, interpoint ,pointB);
		...
		if (compA instanceof Wire && compB instanceof Wire) {
			...
			this.Wires = Wires.filter((comp) => comp != compA && comp != compB);
